...
#include "xenia/gpu/shaders/bytecode/metal/tile_direct_host_resolve.h"
...
MTL::Library* MetalRenderTargetCache::GetOrCreateTileDirectHostResolveLibrary() {
  ...
  NS::Error* error = nullptr;
  dispatch_data_t data = dispatch_data_create(
      tile_direct_host_resolve_metallib,
      sizeof(tile_direct_host_resolve_metallib), nullptr,
      DISPATCH_DATA_DESTRUCTOR_DEFAULT);
  tile_direct_host_resolve_library_ = device_->newLibrary(data, &error);
  dispatch_release(data);
  if (!tile_direct_host_resolve_library_) {
    XELOGE(

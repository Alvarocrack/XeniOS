...
bool SharedMemoryRangeOverlapsSegment(
  ...
  ForEachSharedMemoryRangeSegment(
      range, [&](SharedMemoryRangeSegment range_segment) {
        overlaps = overlaps ||
                   (segment_start < range_segment.end &&
...
bool GetTextureSize(MTL::Texture* texture, uint32_t& width_out,
  ...
  width_out =
      std::max(static_cast<uint32_t>(texture->width()), uint32_t(1));
  height_out =
      std::max(static_cast<uint32_t>(texture->height()), uint32_t(1));
  return true;
}
...
bool GetRenderPassDescriptorSize(MTL::RenderPassDescriptor* pass_descriptor,
  ...
  if (auto* color_attachments = pass_descriptor->colorAttachments()) {
    for (uint32_t i = 0; i < 8; ++i) {
      ...
      if (attachment && GetTextureSize(attachment->texture(), width_out,
                                       height_out)) {
        return true;
      }
    }
...
void GetActiveRenderTargetSize(MTL::RenderPassDescriptor* pass_descriptor,
                               const MetalRenderTargetCache* render_target_cache,
                               uint32_t fallback_width,
                               uint32_t fallback_height, uint32_t& width_out,
                               uint32_t& height_out) {
  width_out = std::max(fallback_width, uint32_t(1));
  height_out = std::max(fallback_height, uint32_t(1));
...
PipelineAttachmentFormats ResolvePipelineAttachmentFormats(
  ...
  // Metal pipeline attachment formats must match the active framebuffer exactly.
  // If no depth attachment is bound, a depth-writing fragment output must not
  // fabricate a depth format in the pipeline descriptor.
...
const char* SharedMemoryRequestReasonName(size_t reason) {
  using Reason = MetalCommandProcessor::SharedMemoryRequestReason;
  switch (static_cast<Reason>(reason)) {
    case Reason::kUnknown:
      return "unknown";
    case Reason::kVertexFetch:
      return "vertex_fetch";
    case Reason::kMemexportStream:
      return "memexport_stream";
    case Reason::kGuestIndex:
      return "guest_index";
    case Reason::kShaderPrimitiveIndex:
      return "shader_primitive_index";
    case Reason::kIndexCopySource:
      return "index_copy_source";
    case Reason::kTextureBase:
      return "texture_base";
    case Reason::kTextureMips:
      return "texture_mips";
...
const char* SharedMemoryPlannerStopReasonName(size_t reason) {
  using Reason = MetalCommandProcessor::SharedMemoryPlannerStopReason;
  switch (static_cast<Reason>(reason)) {
    case Reason::kAlreadyResident:
      return "already_resident";
    case Reason::kUploadBeforeRenderEncoder:
      return "upload_before_render_encoder";
    case Reason::kUploadInsideRenderEncoder:
      return "upload_inside_render_encoder";
    case Reason::kRequestFailed:
      return "request_failed";
    case Reason::kNoSharedMemory:
      return "no_shared_memory";
    case Reason::kTextureDeferredUploadFlush:
...
bool MetalCommandProcessor::SetupContext() {
  ...
  render_encoder_resource_usage_map_.reserve(128);
  render_encoder_heap_usage_.reserve(32);
  render_encoder_heap_usage_set_.reserve(32);
...
void MetalCommandProcessor::MarkSharedMemoryComputeWritePending(
  ...
  XELOGE(
      "MetalCommandProcessor: shared-memory compute write cannot be fenced");
  MarkSharedMemoryWritePending(address, length, MTL::RenderStages(0), false,
                               false);
}
...
void MetalCommandProcessor::MarkSharedMemoryWritePending(
  ...
  for (const PendingSharedMemoryWrite& pending :
    ...
    collapsed.producer_stages = MTL::RenderStages(
        NS::UInteger(collapsed.producer_stages) |
    ...
    collapsed.fence_updated =
        collapsed.fence_updated && pending.fence_updated;
  }
...
void MetalCommandProcessor::UpdateSharedMemoryFenceForActiveRenderEncoder() {
  ...
  if (shared_memory_fence_) {
    current_render_encoder_->updateFence(
        shared_memory_fence_, active_render_encoder_shared_memory_write_stages_);
  }
...
void MetalCommandProcessor::PruneCompletedSharedMemoryWrites(
  ...
  pending_shared_memory_writes_.erase(
      std::remove_if(pending_shared_memory_writes_.begin(),
                     pending_shared_memory_writes_.end(),
                     [completed_submission](
                         const PendingSharedMemoryWrite& pending) {
...
void MetalCommandProcessor::RetireFenceWaitedSharedMemoryWrites(
  ...
  std::array<PendingSharedMemoryWrite,
             kMaxPendingSharedMemoryWriteCapacity>
      retained_entries;
  ...
  for (const PendingSharedMemoryWrite& pending :
    ...
    std::array<SharedMemoryRangeSegment,
               kMaxSharedMemoryWaitSegmentsPerPending>
        remaining_segments;
    std::array<SharedMemoryRangeSegment,
               kMaxSharedMemoryWaitSegmentsPerPending>
        next_segments;
          ...
          ranges[i], [&](SharedMemoryRangeSegment waited_segment) {
            ...
            if (!segment_overflow) {
              for (size_t segment_index = 0; segment_index < next_count;
                   ++segment_index) {
                remaining_segments[segment_index] = next_segments[segment_index];
              }
  ...
  pending_shared_memory_writes_.assign(retained_entries.begin(),
                                       retained_entries.begin() +
                                           retained_count);
  if (pending_shared_memory_writes_.size() <= kMaxPendingSharedMemoryWrites) {
    return;
  }
...
bool MetalCommandProcessor::EncodeSharedMemoryRenderReadDependencies(
  ...
  if (!current_render_encoder_ || !ranges || !range_count ||
      !NS::UInteger(consumer_stages) ||
      pending_shared_memory_writes_.empty()) {
    return true;
  }
  ...
  for (const PendingSharedMemoryWrite& pending :
    ...
    if (pending.active_render_encoder) {
      active_producer_stages = MTL::RenderStages(
          NS::UInteger(active_producer_stages) |
          NS::UInteger(pending.producer_stages));
    } else if (pending.fence_updated && shared_memory_fence_) {
      needs_fence_wait = true;
    } else {
      XELOGE(
          "MetalCommandProcessor: render read cannot wait for shared-memory GPU "
          "writes without a fence");
      return false;
    }
  }
...
bool MetalCommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type,
  ...
  bool fallback_depth_attachment_required =
      pixel_shader_writes_depth_for_fmts;
  ...
  if (memexport_used_pixel && is_rasterization_done) {
    memexport_write_stages = MTL::RenderStages(
  ...
  if (shared_memory_) {
    ...
    auto request_guest_index_range =
        [&](uint64_t index_base, uint32_t index_count,
            xenos::IndexFormat index_format, const char* label,
            SharedMemoryRequestReason reason) -> bool {
      ...
      if (!RequestSharedMemoryRange(reason, static_cast<uint32_t>(index_base),
    ...
    if (guest_dma_index_buffer_read &&
        !request_guest_index_range(
            ...
            SharedMemoryRequestReason::kGuestIndex)) {
      return false;
    }
    if (shader_primitive_index_load &&
        !request_guest_index_range(
            ...
            "shader primitive",
            SharedMemoryRequestReason::kShaderPrimitiveIndex)) {
      return false;
    }
  ...
  if (prepare_uniforms) {
    ...
    if (!PrepareDrawConstants(
            ...
            metal_pixel_shader, shared_memory_is_uav, is_rasterization_done,
  ...
  auto add_shared_memory_hazard_range = [&](uint32_t start, uint32_t length) {
    if (!length ||
        shared_memory_hazard_range_count >= shared_memory_hazard_ranges.size()) {
      return;
    }
  ...
  shared_memory_consumer_stages = MTL::RenderStages(
      NS::UInteger(shared_memory_consumer_stages) |
      NS::UInteger(memexport_write_stages));
  if (shared_memory_hazard_range_count &&
      PendingSharedMemoryWritesOverlapRanges(
          shared_memory_hazard_ranges.data(),
          shared_memory_hazard_range_count)) {
    if (!EncodeSharedMemoryRenderReadDependencies(
            shared_memory_hazard_ranges.data(), shared_memory_hazard_range_count,
            shared_memory_consumer_stages)) {
      return false;
    }
    if (!current_render_encoder_) {
  ...
  if (prepare_uniforms) {
    ApplyDrawDynamicState(draw_dynamic_state);
  ...
  return DispatchDraw(regs, primitive_processing_result,
                      ...
                      memexport_write_stages, uses_vertex_fetch,
                      prepared_guest_dma_index_buffer.buffer
                          ? &prepared_guest_dma_index_buffer
                          : nullptr,
                      vb_bindings, vertex_ranges.data(), vertex_range_count,
                      index_buffer_info);
}
...
void MetalCommandProcessor::ApplyDrawDynamicState(
  ...
  if (dynamic_state.rasterization_enabled) {
    ApplyRasterizerState(dynamic_state.primitive_polygonal);
  }
...
bool MetalCommandProcessor::PopulateBindlessTables(
  ...
  if (render_encoder_bindless_stable_resources_serial_ !=
    ...
    auto track_texture_usage = [&](MTL::Texture* texture) {
      ...
      if (textures_for_encoder_count >= textures_for_encoder.size()) {
        return;
      }
      textures_for_encoder[textures_for_encoder_count++] = texture;
    };

    auto track_texture_usages = [&](const std::vector<MTL::Texture*>& textures) {
      for (MTL::Texture* texture : textures) {
        track_texture_usage(texture);
      }
    };

    if (metal_vertex_shader) {
      track_texture_usages(current_texture_bindless_resources_vertex_);
    }
    if (metal_pixel_shader) {
  ...
  if (root_resources_need_update) {
    ...
    auto track_uniform_buffer_usage = [&](MTL::Buffer* uniform_buffer) {
      ...
      assert_true(uniform_buffer_count < uniform_buffers_for_encoder.size());
...
bool MetalCommandProcessor::PrepareGuestDMAIndexBufferForMemexport(
  ...
  if (constant_buffer_pool_ &&
    ...
    (void)constant_buffer_pool_->Request(
        GetCurrentSubmission(), static_cast<size_t>(copy_size), 4,
        &scratch_buffer, scratch_offset, scratch_gpu_address);
  }
  if (!scratch_buffer) {
    direct_scratch_buffer =
        device_->newBuffer(static_cast<NS::UInteger>(copy_size),
...
void MetalCommandProcessor::MaybeDumpBackendTelemetry(const char* reason,
  ...
  auto format_shared_memory_request_triplets =
      [](const std::array<uint64_t, kSharedMemoryRequestReasonCount>& total,
         const std::array<uint64_t, kSharedMemoryRequestReasonCount>& active,
         const std::array<uint64_t, kSharedMemoryRequestReasonCount>&
             no_active) {
        ...
        for (size_t i = 0; i < total.size(); ++i) {
          ...
          formatted +=
              fmt::format("{}={}/{}/{}", SharedMemoryRequestReasonName(i),
                          total[i], active[i], no_active[i]);
        }
        if (formatted.empty()) {
          formatted = "none";
        }
  ...
  auto format_shared_memory_request_counts =
      [](const std::array<uint64_t, kSharedMemoryRequestReasonCount>& values) {
        std::string formatted;
        for (size_t i = 0; i < values.size(); ++i) {
          ...
          formatted +=
              fmt::format("{}={}", SharedMemoryRequestReasonName(i), values[i]);
        }
        if (formatted.empty()) {
          formatted = "none";
        }
      ...
      [](const std::array<uint64_t, kSharedMemoryRequestReasonCount>& before,
        ...
        for (size_t i = 0; i < before.size(); ++i) {
          ...
          formatted += fmt::format("{}={}/{}",
                                   SharedMemoryRequestReasonName(i), before[i],
                                   after[i]);
        }
        if (formatted.empty()) {
          formatted = "none";
  ...
  std::string shared_memory_request_calls =
      format_shared_memory_request_triplets(
          backend_telemetry_.shared_memory_request_calls_total,
          backend_telemetry_.shared_memory_request_calls_active,
          backend_telemetry_.shared_memory_request_calls_no_active);
  std::string shared_memory_request_input_ranges =
      format_shared_memory_request_counts(
          backend_telemetry_.shared_memory_request_input_ranges);
  ...
  std::string shared_memory_request_upload_calls =
      format_shared_memory_request_triplets(
          ...
          backend_telemetry_.shared_memory_request_upload_calls_active,
          backend_telemetry_.shared_memory_request_upload_calls_no_active);
  std::string shared_memory_request_upload_pages =
  ...
  std::string shared_memory_planner_stop_reasons;
  for (size_t i = 0;
       i < backend_telemetry_.shared_memory_planner_stop_reasons.size(); ++i) {
    uint64_t count = backend_telemetry_.shared_memory_planner_stop_reasons[i];
    if (!count) {
      continue;
    }
    ...
    shared_memory_planner_stop_reasons += fmt::format(
        "{}={}", SharedMemoryPlannerStopReasonName(i), count);
  }
  if (shared_memory_planner_stop_reasons.empty()) {
  ...
  XELOGI(
      ...
      "crossed/overlap/texture_skip={}/{}/{} event_write_shd "
      "crossed/overlap={}/{} stop_opcodes={{ {} }}",
      reason,
      ...
      backend_telemetry_.render_run_planner_event_write_shd_crossed,
      backend_telemetry_.render_run_planner_event_write_shd_overlap_stops,
      render_run_planner_event_query_stop_opcodes);
  XELOGI(
...
bool MetalCommandProcessor::RequestSharedMemoryRanges(
  ...
  if (reason_valid) {
    ++backend_telemetry_.shared_memory_request_calls_total[reason_index];
    if (was_active) {
      ++backend_telemetry_.shared_memory_request_calls_active[reason_index];
    } else {
      ++backend_telemetry_.shared_memory_request_calls_no_active[reason_index];
    }
    backend_telemetry_.shared_memory_request_input_ranges[reason_index] +=
        range_count;
  }

  if (!shared_memory_) {
    RecordSharedMemoryPlannerStop(SharedMemoryPlannerStopReason::kNoSharedMemory);
    if (reason_valid) {
  ...
  if (reason_valid) {
    ...
    backend_telemetry_
        .shared_memory_request_upload_page_ranges_after_coalesce[reason_index] +=
  ...
  if (!success) {
    RecordSharedMemoryPlannerStop(SharedMemoryPlannerStopReason::kRequestFailed);
  } else if (!stats.upload_bytes) {
    RecordSharedMemoryPlannerStop(SharedMemoryPlannerStopReason::kAlreadyResident);
  } else if (was_active) {
    RecordSharedMemoryPlannerStop(
        SharedMemoryPlannerStopReason::kUploadInsideRenderEncoder);
  } else {
    RecordSharedMemoryPlannerStop(
        SharedMemoryPlannerStopReason::kUploadBeforeRenderEncoder);
  }
...
void MetalCommandProcessor::WarmVertexFetchSharedMemoryBeforeRenderPass(
  ...
  auto scan_warm_ranges = [&]() {
    ...
    struct CrossedGuestWriteRange {
      SharedMemory::Range range;
      uint32_t opcode;
    };
    std::array<CrossedGuestWriteRange, kMetalVertexFetchWarmMaxRanges>
    ...
    auto append_crossed_guest_write_range = [&](uint32_t opcode,
                                                uint32_t start,
                                                uint32_t length) {
      if (!length) {
        return true;
      ...
      for (uint32_t i = 0; i < crossed_guest_write_range_count;) {
        if (crossed_guest_write_ranges[i].opcode != opcode) {
          ++i;
          continue;
        }
        uint32_t existing_start = crossed_guest_write_ranges[i].range.start;
        uint32_t existing_end =
            existing_start + crossed_guest_write_ranges[i].range.length;
      ...
      crossed_guest_write_ranges[crossed_guest_write_range_count++] = {
          {merged_start, merged_end - merged_start}, opcode};
      return true;
    };
    auto overlapping_crossed_guest_write_opcode = [&](uint32_t start,
                                                      uint32_t length) {
      for (uint32_t i = 0; i < crossed_guest_write_range_count; ++i) {
        const CrossedGuestWriteRange& write_range =
            crossed_guest_write_ranges[i];
        if (ranges_overlap(start, length, write_range.range.start,
                           write_range.range.length)) {
          return write_range.opcode;
        }
      }
      return uint32_t(0);
    };
    auto read_overlaps_crossed_guest_write = [&](uint32_t start,
                                                 uint32_t length) {
      return overlapping_crossed_guest_write_opcode(start, length) != 0;
    };
    ...
    auto append_invalid_range =
        [&](std::array<SharedMemory::Range, kMetalVertexFetchWarmMaxRanges>&
          ...
          uint32_t crossed_write_opcode =
              overlapping_crossed_guest_write_opcode(start, length);
          if (crossed_write_opcode) {
            if (crossed_write_opcode == xenos::PM4_EVENT_WRITE_EXT) {
              ++backend_telemetry_
                    .render_run_planner_event_write_ext_overlap_stops;
            } else if (crossed_write_opcode == xenos::PM4_EVENT_WRITE_SHD) {
              ++backend_telemetry_
                    .render_run_planner_event_write_shd_overlap_stops;
            }
            stop_event_or_query(crossed_write_opcode);
            return false;
          }
    ...
    auto append_future_dma_index_range = [&]() {
      ...
      uint32_t guest_index_base = warm_regs[XE_GPU_REG_VGT_DMA_BASE] &
                                  ~uint32_t((1u << index_size_log2) - 1);
      uint32_t guest_index_bytes = guest_index_count << index_size_log2;
      if (guest_index_base > SharedMemory::kBufferSize ||
          SharedMemory::kBufferSize - guest_index_base < guest_index_bytes) {
        return stop_with(VertexFetchWarmerStopReason::kUnsupportedPacket);
      }
      return append_invalid_range(result.index_ranges, result.index_range_count,
    ...
    auto append_future_draw_ranges = [&]() {
      ...
      ++result.draw_count;
      if (result.draw_count >= kMetalVertexFetchWarmMaxDraws) {
      ...
      switch (opcode) {
        ...
        case xenos::PM4_EVENT_WRITE_EXT: {
          ...
          if (address < SharedMemory::kBufferSize) {
            ...
            if (!append_crossed_guest_write_range(
                    xenos::PM4_EVENT_WRITE_EXT, address, length)) {
              goto finish_parse;
            }
          }
          ++backend_telemetry_.render_run_planner_event_write_ext_crossed;
        ...
        case xenos::PM4_EVENT_WRITE_SHD: {
          ...
          address &= ~uint32_t(0x3);
          constexpr uint32_t kEventWriteShdLength = sizeof(uint32_t);
          if (address < SharedMemory::kBufferSize) {
            uint32_t length =
                std::min(kEventWriteShdLength,
                         uint32_t(SharedMemory::kBufferSize - address));
            if (!append_crossed_guest_write_range(
                    xenos::PM4_EVENT_WRITE_SHD, address, length)) {
              goto finish_parse;
            }
          }
          ++backend_telemetry_.render_run_planner_event_write_shd_crossed;
  ...
  if (scan_result.vertex_range_count &&
      !RequestSharedMemoryRanges(SharedMemoryRequestReason::kVertexFetch,
                                 scan_result.vertex_ranges.data(),
                                 scan_result.vertex_range_count)) {
    finish(VertexFetchWarmerStopReason::kRequestFailed, false);
    return;
  }
  if (scan_result.index_range_count &&
      !RequestSharedMemoryRanges(SharedMemoryRequestReason::kGuestIndex,
                                 scan_result.index_ranges.data(),
                                 scan_result.index_range_count)) {
    finish(VertexFetchWarmerStopReason::kRequestFailed, false);
    return;
  }
  backend_telemetry_.vertex_fetch_warmer_draws += scan_result.draw_count;
  backend_telemetry_.vertex_fetch_warmer_ranges +=
      scan_result.vertex_range_count + scan_result.index_range_count;
  backend_telemetry_.vertex_fetch_warmer_index_ranges +=
      scan_result.index_range_count;
  finish(scan_result.stop_reason, true);
}
...
void MetalCommandProcessor::UseRenderEncoderHeap(MTL::Heap* heap) {
  ...
  if (render_encoder_heap_usage_set_.empty()) {
    for (MTL::Heap* used_heap : render_encoder_heap_usage_) {
      render_encoder_heap_usage_set_.insert(used_heap);
    }
  }
...
MetalCommandProcessor::GetDrawRenderPassDescriptor(
    bool fallback_depth_attachment_required) {
  if (render_target_cache_) {
    if (MTL::RenderPassDescriptor* cache_desc =
            render_target_cache_->GetRenderPassDescriptor(
